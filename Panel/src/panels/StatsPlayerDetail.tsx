...
interface Props { steamId: string; selfSteamId?: string; onBack: () => void; }

export default function StatsPlayerDetail({ steamId, selfSteamId, onBack }: Props) {
  const [d, setD] = useState<Cs2ssPlayerDetailResponse | null>(null);
  const [loading, setLoading] = useState(true);
  const [err, setErr] = useState("");

  useEffect(() => {
    api.getCs2ssPlayerDetail("", steamId).then(r => { setD(r); setLoading(false); }).catch(e => { setErr(String(e)); setLoading(false); });
  }, [steamId]);

  if (loading) return <div className="stats-panel"><div className="stats-panel__loading">Loading…</div></div>;
  if (err || !d) return <div className="stats-panel"><div className="stats-panel__error">{err || "No data"}</div></div>;
  ...
  const radar = [
    { s: "Rating", v: Math.max(0, avgR) / 2 }, { s: "K/D", v: Math.min(kd / 3, 1) },
    { s: "ADR", v: Math.min(cs2ssCalcAdr(total.damage, total.rounds) / 150, 1) },
    { s: "KPR", v: Math.min(total.kills / Math.max(1, total.rounds), 1) },
    { s: "HS%", v: cs2ssCalcHsPct(total.headshots, total.kills) / 100 }, { s: "Win%", v: wr / 100 },
  ];
  ...
  return (
    <div className="stats-panel">
      ...
      <div className="stats-hero">
        <div>
          <span className="stats-hero__eyebrow">PLAYER DOSSIER</span>
          <h1>{name}{steamId === selfSteamId ? <span style={{ color: "#a99aff", fontSize: 14, marginLeft: 10, fontWeight: 400 }}>(You)</span> : ""}</h1>
      ...
      <div className="stats-charts">
        <div className="stats-panel-block">
          <div className="stats-panel-block__title"><div><span>RADAR</span><h2>Performance Radar</h2></div></div>
      ...
      <div className="stats-panel-block" style={{ padding: 0 }}>
        <div style={{ padding: "20px 24px 0" }}><span style={{ fontSize: 10, fontWeight: 900, letterSpacing: ".18em", color: "var(--c-accent)" }}>MAPS</span><h2 style={{ margin: "4px 0 16px", fontSize: 18, fontWeight: 700 }}>Map Performance</h2></div>
        <table className="stats-table"><thead><tr><th>Map</th><th style={{ textAlign: "right" }}>Matches</th><th style={{ textAlign: "right" }}>K/D</th><th style={{ textAlign: "right" }}>ADR</th><th style={{ textAlign: "right" }}>Rating</th></tr></thead>
      ...
      <div className="stats-panel-block" style={{ padding: 0 }}>
        <div style={{ padding: "20px 24px 0" }}><span style={{ fontSize: 10, fontWeight: 900, letterSpacing: ".18em", color: "var(--c-accent)" }}>HISTORY</span><h2 style={{ margin: "4px 0 16px", fontSize: 18, fontWeight: 700 }}>Match History</h2></div>
        <table className="stats-table"><thead><tr><th>Map</th><th>Date</th><th>Score</th><th style={{ textAlign: "right" }}>K/D/A</th><th style={{ textAlign: "right" }}>ADR</th><th style={{ textAlign: "right" }}>HS%</th><th style={{ textAlign: "right" }}>Rating</th></tr></thead>
