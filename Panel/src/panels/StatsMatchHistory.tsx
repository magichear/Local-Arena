  ...
  return (
    <div className="stats-panel">
      <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 22 }}>
        {onBack && <button className="stats-back" onClick={onBack}>← Back</button>}
        <span style={{ fontSize: 13, color: "var(--text-secondary)", fontWeight: 600 }}>{filtered.length} of {matches.length} matches</span>
      </div>

      <div className="stats-filters">
        <label><span>Map</span><select value={mapF} onChange={e => setMapF(e.target.value)}><option value="">All</option>{maps.map(m => <option key={m} value={m}>{cs2ssMapLabel(m)}</option>)}</select></label>
        <label><span>Mode</span><select value={modeF} onChange={e => setModeF(e.target.value)}><option value="all">All</option><option value="competitive">Competitive</option><option value="deathmatch">Deathmatch</option></select></label>
        <label><span>From</span><input type="date" value={dateF} onChange={e => setDateF(e.target.value)} /></label>
        <label><span>To</span><input type="date" value={dateT} onChange={e => setDateT(e.target.value)} /></label>
        <button onClick={() => { setMapF(""); setModeF("all"); setDateF(""); setDateT(""); }}>Reset</button>
      </div>

      <div className="stats-panel-block" style={{ padding: 0 }}>
        <table className="stats-table">
          <thead><tr><th>Map</th><th>Date</th><th>Score</th><th>Rounds</th><th style={{ textAlign: "right" }}>K/D/A</th><th style={{ textAlign: "right" }}>ADR</th><th style={{ textAlign: "right" }}>Rating</th></tr></thead>
          <tbody>
            {filtered.length === 0 ? (
              <tr><td colSpan={7} style={{ textAlign: "center", padding: 40, color: "var(--text-secondary)" }}>No matches match the filters</td></tr>
            ) : filtered.map(m => {
              ...
              return (
                <tr key={m.matchId} onClick={() => onOpenMatch?.(m.matchId)} style={{ cursor: "pointer" }}>
                  ...
                  <td>
                    {dm ? (
                      <span style={{ color: "#df6b35", fontWeight: 700 }}>{Math.round(m.durationSeconds / 60)} min</span>
                    ) : (
                  ...
                  <td>{dm ? `${Math.round(m.durationSeconds / 60)}m` : `${m.roundsPlayed} rds`}</td>
